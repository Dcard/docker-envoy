...
RUN echo 'INPUT(-l:libc++.a -l:libc++abi.a -lm -lpthread)' > /usr/lib/$(uname -p)-linux-gnu/libc++.so

# Install CMake
ENV CMAKE_VERSION=3.22.0
RUN curl -L https://github.com/Kitware/CMake/releases/download/v${CMAKE_VERSION}/cmake-${CMAKE_VERSION}-linux-$(uname -p).tar.gz | tar zxf - -C /usr/local
RUN ln -s /usr/local/cmake-${CMAKE_VERSION}-linux-$(uname -p)/bin/cmake /usr/local/bin/cmake
